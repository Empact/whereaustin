...
var wifi_icon = new GIcon(base_icon)
wifi_icon.iconSize = new GSize(16, 16);
...
var wifi_online_icon = new GIcon(wifi_icon);
wifi_online_icon.image = "/images/wifi_online.png";

var wifi_flakey_icon = new GIcon(wifi_icon);
wifi_flakey_icon.image = "/images/wifi_flakey.png";

var wifi_offline_icon = new GIcon(wifi_icon);
wifi_offline_icon.image = "/images/wifi_offline.png";

var wifi_low_usage_icon = new GIcon(wifi_icon);
...
function getIcon(type)
{
	...
	case "Low Usage":    icon = wifi_low_usage_icon; break;
	default:
...
function livesearch()
{
	query = $('whereaustinsearch').value;
	var venuelist = $('sxswul').getElementsByTagName('li');
	var venues = $A(venuelist);
...
/*
         tmp=''
         for(x=0;x<i[a].getElementsByTagName('td').length;x++){
  		 tmp+= so_getText(i[a].getElementsByTagName('td')[x]);
         }
	
		var reg = new RegExp(ft.value, "i");
		if (reg.test(tmp))
		{
      	  i[a].style.display='';
          dataQty++;
		  $('numresults').textContent = dataQty;
